...
import REQUESTED_MEDICATION from "@salesforce/schema/VCC_Progress_Note__c.VCC_Medications_Refill_Renewal_Request__c";
...
//need to add customPermission and if else statement to meet tier 2 3
const columns = [
    //second original fields (Teir 1 and 2 not Triage)
    //Components to display for for Pharma Tier I, II, III Persona (to be tested in Sandbox when connection establishes)
...
export default class VccProgressNoteMedsModal extends LightningElement {
    /** Properties */
    @track displayList;
    @track displayStatus;
    @track medField;
    ...
    preTextValue = "Rx # - Medication - Fill Qty - Last Fill Date - SIG - Provider Comments - Status \n";

    addToNote(medsToAdd) {
        //let medField = this.template.querySelector('lightning-input-field[data-field-name="VCC_Requested_Medication__c"]');
        let medField = this.template.querySelector('lightning-input-field[data-field-name="VCC_Medications_Refill_Renewal_Request__c"]');
    ...
    handleAddSelectedRenewal(event) {
        ...
        console.log("This is the datatable: ", proxyTool(datatable));
        console.log("These are the meds: ", proxyTool(this.meds));
