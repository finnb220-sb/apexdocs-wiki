...
export default class Popovers extends LightningElement {
    ...
    handleSlot() {
        [...this.template.querySelectorAll("slot")].forEach((slot) => {
            let hasSlotValue = slot.assignedElements().length !== 0 && slot.outerText.length !== 0;
    ...
    addPopoverClass() {
        ...
        style += this.variant && VARIANTS[this.variant] ? VARIANTS[this.variant] : "";
        style += this.size && POPOVER_SIZES[this.size] ? POPOVER_SIZES[this.size] : POPOVER_SIZES["medium"];
    ...
    handleMouseOverOrFocusElement(event) {
        ...
        this.popoverStyle =
            "position: absolute; left: " + popoverStyleOptions.popoverXPos + "px; top: " + popoverStyleOptions.popoverYPos + "px;visibility: visible;opacity: 1;z-index: 9999;";
    }
